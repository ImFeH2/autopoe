...
export type HuddolChannel = {
  onmessage: (message: unknown) => void;
};

export type HuddolEventListener = (event: string, data: unknown) => void;

export class HuddolRequestError extends Error {
  readonly code: string;

  constructor(code: string, message: string) {
    super(message);
    this.name = "HuddolRequestError";
    this.code = code;
  }
}
...
type HuddolClientOptions = {
  channelFactory?: () => HuddolChannel;
...
export class HuddolClient {
  private readonly channelFactory: () => HuddolChannel;
  private readonly invokeCommand: InvokeCommand;
  private readonly eventListeners = new Set<HuddolEventListener>();
  ...
  onEvent(listener: HuddolEventListener): () => void {
  ...
  private receive(message: unknown) {
    ...
    this.rejectPending(
      id,
      new HuddolRequestError(responseCode, responseMessage),
    );
  }
...
export const huddol = new HuddolClient();
