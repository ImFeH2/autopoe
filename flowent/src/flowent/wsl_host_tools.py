...
from flowent.host_tools import AgentHostTools, HostToolError, HostTools

HOST_BACKEND_ENV = "FLOWENT_HOST_BACKEND"
HOST_BINARY_ENV = "FLOWENT_WSL_HOST_BINARY"
WORKING_DIRECTORY_ENV = "FLOWENT_WORKING_DIRECTORY"
...
def create_host_tools() -> AgentHostTools:
    ...
    if preference not in ("auto", "native", "wsl"):
        raise RuntimeError("FLOWENT_HOST_BACKEND must be auto, native, or wsl")
...
def resolve_host_binary() -> Path:
    if override := os.environ.get(HOST_BINARY_ENV):
        ...
        candidate = Path(bundle_root) / "flowent-host"
    else:
        candidate = (
...
def install_host_binary(probe: WslProbe, source: Path) -> str:
    ...
    directory = f"{probe.home}/.cache/flowent/host/{digest}"
    destination = f"{directory}/flowent-host"
    _run_wsl(
        [
            "wsl.exe",
            "--distribution",
