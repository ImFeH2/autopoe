...
pub fn create_restricted(capability_sid: &str) -> AppResult<RestrictedIdentity> {
    ...
    grant_default_object_access(&restricted, &capability)?;
...
fn grant_default_object_access(token: &OwnedHandle, sid: &[u8]) -> AppResult<()> {
...
mod tests {
    ...
    use super::create_restricted;
    use crate::windows::util::sid_to_string;

    #[test]
    fn restricted_identity_keeps_logon_access() {
        let identity = create_restricted("S-1-5-21-1-2-3-4").unwrap();

        assert_ne!(unsafe { IsTokenRestricted(identity.token.get()) }, 0);
        assert!(
            sid_to_string(&identity.logon_sid)
                .unwrap()
                .starts_with("S-1-5-5-")
        );
    }
