...
LOG_FILE_NAME = "huddol.jsonl"
MAX_LOG_BYTES = 10 * 1024 * 1024
BACKUP_COUNT = 5

_logger = logging.getLogger("huddol.diagnostics")
...
class JsonFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord) -> str:
        fields = getattr(record, "huddol_fields", {})
        payload = {
            "timestamp": datetime.fromtimestamp(record.created, UTC).isoformat(),
            "level": record.levelname,
            "event": getattr(record, "huddol_event", "diagnostics.unknown"),
...
def log_event(event: str, *, level: int = logging.INFO, **fields: Any) -> None:
    ...
    _logger.log(
        level,
        event,
        extra={"huddol_event": event, "huddol_fields": safe_fields},
    )
