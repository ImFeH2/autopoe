...
def test_huddol_runs_until_stdin_closes(tmp_path: Path) -> None:
    data = tmp_path / "data"
    process = start_huddol(data)
    ...
    records = [
        json.loads(line)
        for line in (data / "logs" / "huddol.jsonl").read_text().splitlines()
    ]
    protocol_stop = next(
...
def test_huddol_shutdown_request_stops_the_process(tmp_path: Path) -> None:
    data = tmp_path / "data"
    process = start_huddol(data)
...
def test_huddol_writes_private_diagnostics_without_request_content(
    tmp_path: Path,
) -> None:
    data = tmp_path / "data"
    process = start_huddol(data)
    ...
    log_path = data / "logs" / "huddol.jsonl"
...
def test_huddol_accepts_utf8_jsonl_messages(tmp_path: Path) -> None:
...
def test_memory_stays_in_data_directory_and_orphans_are_removed_on_startup(
    ...
    process = start_huddol(data, workspace)
    try:
        assert request(process, 1, "system.shutdown", {}) == {"stopped": True}
        assert process.wait(timeout=10) == 0
    finally:
...
def test_huddol_does_not_load_model_settings_from_dotenv(tmp_path: Path) -> None:
    ...
    process = start_huddol(tmp_path / "data", working_directory)
...
def test_persists_state_and_uses_each_launch_directory(
    ...
    second = start_huddol(data, second_directory)
    try:
...
def test_agent_model_history_continues_across_process_restarts(
    ...
    (support / "sitecustomize.py").write_text(
        "import huddol.model_runner as model_runner\n"
        "from pydantic_ai.messages import ModelRequest,ModelResponse,TextPart,UserPromptPart\n"
        "from huddol.runtime import AgentRunOutcome\n"
        ...
        "if context.message_history else 'huddol-agent-2')\n"
    ...
    second = start_huddol(data, tmp_path, environment)
    try:
        snapshot = request(
            second,
            1,
...
def test_hard_killed_huddol_cleans_active_run(tmp_path: Path) -> None:
