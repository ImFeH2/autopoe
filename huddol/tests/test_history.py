...
from huddol.domain import DomainError, Reminder, ReminderMention
...
def test_persists_complete_agent_history_and_restores_model_messages(
    ...
    messages = (
        ModelRequest(
            parts=[
                SystemPromptPart(content="You are a Huddol Agent"),
    ...
    assert first_run["entries"][1]["content"] == "You are a Huddol Agent"
...
def test_native_web_search_is_visible_and_searchable_after_compaction(
            ...
            ModelResponse(
                parts=[
                    NativeToolCallPart(
                        ...
                        args={"query": "Huddol agent collaboration"},
    ...
    assert "Huddol agent collaboration" in first_run["entries"][1]["content"]
    assert "https://example.com/huddol" in first_run["entries"][2]["content"]
    ...
    searched = history.search_compacted(2, "example.com/huddol")
...
def test_merges_live_text_deltas_and_publishes_ordered_events(tmp_path: Path) -> None:
    ...
    run.emit("thinking", part_id="0-0")
    run.emit("text_delta", part_id="0-1", content="Hud")
    run.emit("text_delta", part_id="0-1", content="dol")
    ...
    assert active["entries"][2]["content"] == "Huddol"
