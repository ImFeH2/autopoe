    ...
    pub fn main() -> Result<(), String> {
        ...
        for line in stdin.lock().lines() {
            ...
            let request: Request = match serde_json::from_str(&line) {
                Ok(request) => request,
                Err(error) => {
                    eprintln!("[flowent-host] Invalid request: {error}");
                    continue;
                }
            };
        ...
        let result = (|| {
            ...
            for attempt in 0..100 {
                let path = parent.join(format!(
                    ".{name}.flowent-{}-{request_id}-{attempt}",
                    std::process::id()
                ));
    ...
    mod tests {
        ...
        fn temporary_directory() -> PathBuf {
            let suffix = SystemTime::now()
                ...
                .unwrap()
                .as_nanos();
            let path = env::temp_dir().join(format!("flowent-host-test-{suffix}"));
...
#[cfg(not(unix))]
fn main() {
    eprintln!("flowent-host requires a Unix environment");
    std::process::exit(1);
}
